...
void Localization::cb_lidar_scan(const sensor_msgs::PointCloud2 &msg) {
  ...
  pass1.setInputCloud(bag_pointcloud);
  ...
  outFile << pub_count + 1 << ','<< odom.pose.pose.position.x << ',' << odom.pose.pose.position.y << ',' << odom.pose.pose.position.z <<','<< yaw << ',' << pitch << ',' << roll << endl;
  pub_count++;
  if (pub_count==396){
