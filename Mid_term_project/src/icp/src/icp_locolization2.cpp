...
icp_localization::icp_localization() {
  ...
  pass1.setInputCloud(map);
  pass1.setFilterFieldName("y");
  pass1.setFilterLimits(0, 950.0);
  pass1.filter(*map);//儲存處理之後的點雲
  // pass1.setInputCloud(map);
  // pass1.setFilterFieldName("z");
  // pass1.setFilterLimits(1, 7.0);
  // pass1.filter(*map);//儲存處理之後的點雲
  ...
  sub_lidar_scan = nh.subscribe("lidar_points", 50, &icp_localization::cb_lidar_scan, this);
  ...
  int init_x= 1774.8362264986283;
  int init_y= 866.367052112287;
  int init_z= 0.015302029201818898;
  double yaw=0 ;
...
void icp_localization::cb_lidar_scan(const sensor_msgs::PointCloud2 &msg) {
  ...
  */
  ...
  // voxel.setFilterFieldName("x");
  // voxel.setFilterLimits(-30.0, 30.0);
  ...
  pass.filter(*bag_pointcloud);//儲存處理之後的點雲
  
  cout<<"voxel grid filter: "<<bag_pointcloud->points.size()<<endl;
  pcl::PassThrough<pcl::PointXYZI> pass2;
  pass2.setInputCloud(bag_pointcloud);
  pass2.setFilterFieldName("y");
  pass2.setFilterLimits(-30.0, 40.0);
  pass2.filter(*bag_pointcloud);//儲存處理之後的點雲
